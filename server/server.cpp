    ...
    while (true)
    {
        ...
        for (int i = 0; i < nfds; i++)
        {
            if (events[i].events & EPOLLIN)
            {
                ...
                cout << "Received " << bytes_received << " bytes from client " << activate_fd << endl;
...
void onClientDisconnected(int client_fd)
{
    ...
    if (!matchCode.empty() && waitingClients.count(matchCode) && waitingClients[matchCode] == client_fd)
    {
...
void onMessageReceived(int client_fd, void *buffer, int bytes)
{
    ...
    { // 假设匹配码是4个字符
        ...
        cout << "Received match code from client " << client_fd << ": " << message << endl;
        ...
        if (waitingClients.count(message))
        {
            ...
            cout << "Match found between " << client_fd << " and " << waitingClientFd << endl;
        ...
        {
            cout << "No match found for client " << client_fd << " Waiting..." << endl;
            // 没有找到匹配，加入等待队列
    ...
    else
    {
        ...
        string info = "Invalid message format received from client " + to_string(client_fd);
        cout << info << endl;
