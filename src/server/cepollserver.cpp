...
bool CEpollServer::createServerSocket(const string &ip, int port)
{
  ...
  { // SOMAXCONN is a global variable that defines the maximum length for the queue of pending connections, default is 128
...
bool CEpollServer::handleNewConnection()
{
  ...
  int client_fd = accept(server_fd, (sockaddr *)&addr, &addr_len); // Accept client connection
  if (client_fd == -1)
  {
...
{
  void *buffer[1024];
  int bytes = recv(client_fd, buffer, sizeof(buffer), 0); // Receive message from client
  if (bytes <= 0)
  {
    if (bytes == 0)
    {
    ...
    {
      onClientDisconnected(client_fd); // Call client disconnection event callback
    }
  ...
  else
  {
    ...
    {
      onMessageReceived(client_fd, buffer, bytes); // Call message received event callback
    }
  }
  return true;
};
...
void CEpollServer::run()
{
  while (true)
  {
    int nfds = epoll_wait(epoll_fd, events, max_events, -1); // Wait for events to occur
    if (nfds == -1)
    {
